...
public class TileEntityBreaker extends TileEntityLockable implements ITickable, ISidedInventory {
	...
	public static void registerFixesFurnace(DataFixer fixer) {
		fixer.registerWalker(FixTypes.BLOCK_ENTITY,
				new ItemStackDataLists(TileEntityBreaker.class, new String[] { "Items" }));
	}
	...
	private boolean canSmelt() {
		if (((ItemStack) this.breakerItemStacks.get(0)).isEmpty()) {
			...
			ItemStack itemstack = BreakerRecipes.getInstance().getBreakingResult(this.breakerItemStacks.get(0));
	...
	public void smeltItem() {
		if (this.canSmelt()) {
			...
			if (itemstack.getItem() == Item.getItemFromBlock(Blocks.SPONGE) && itemstack.getMetadata() == 1
					&& !((ItemStack) this.breakerItemStacks.get(1)).isEmpty()
					&& ((ItemStack) this.breakerItemStacks.get(1)).getItem() == Items.BUCKET) {
	...
	public Container createContainer(InventoryPlayer playerInventory, EntityPlayer playerIn) {
		return new BreakDownContainer(playerInventory, this);
	}
