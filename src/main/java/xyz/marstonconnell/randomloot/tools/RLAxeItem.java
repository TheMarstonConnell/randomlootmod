...
public class RLAxeItem extends RLToolItem implements IRLTool {
	 ...
	 private static final Set<Material> EFFECTIVE_ON_MATERIALS = Sets.newHashSet(Material.WOOD, Material.NETHER_WOOD, Material.PLANTS, Material.TALL_PLANTS, Material.BAMBOO, Material.GOURD);
	   public static final Set<Block> EFFECTIVE_ON_BLOCKS = Sets.newHashSet(Blocks.LADDER, Blocks.SCAFFOLDING, Blocks.OAK_BUTTON, Blocks.SPRUCE_BUTTON, Blocks.BIRCH_BUTTON, Blocks.JUNGLE_BUTTON, Blocks.DARK_OAK_BUTTON, Blocks.ACACIA_BUTTON, Blocks.CRIMSON_BUTTON, Blocks.WARPED_BUTTON);
	...
	public List<BasicTag> getAllowedTags() {
		...
		for (BasicTag tag : TagHelper.allTags) {
			if(tag.forTools || tag.forWeapons) {
				allowedTags.add(tag);
			}
		}
		return allowedTags;
